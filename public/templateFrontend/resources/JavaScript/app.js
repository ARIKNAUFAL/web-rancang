...
const btnMenu = document.querySelector("header>button:first-child");

// **remove all the hrs in categories section at min-width 768px
...
removeHr(minWidth768); // Call listener function at run time
minWidth768.addListener(removeHr); // Attach listener function on state changes

// **change the contents inside header tag at min-width 768px

function removeInHeader(width) {
    if (width.matches) {
        btnMenu.remove();
    }
}

removeInHeader(minWidth768); // Call listener function at run time
minWidth768.addListener(removeInHeader); // Attach listener function on state changes

// **add difference contents inside the header at min-width 768px

// **create the h2 element
const h2 = document.createElement("h2");
h2.append("GuidanceTutorial");

// **create the My Learning link
const myLearningLink = document.createElement("a");
myLearningLink.setAttribute("href", "/my-learning");
...
// **create the Notification link
const notificationLink = document.createElement("a");
notificationLink.setAttribute("href", "/notifications");
...
notification.setAttributeNS(
    null,
    "viewBox",
    "0 0 " + boxWidthNotif + " " + boxHeightNotif
);
...
loginState.setAttributeNS(
    null,
    "viewBox",
    "0 0 " + boxWidthLoginState + " " + boxHeightLoginState
);
...
addInHeader(minWidth768); // Call listener function at run time
minWidth768.addListener(addInHeader); // Attach listener function on state changes
